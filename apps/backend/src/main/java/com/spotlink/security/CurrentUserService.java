...
public class CurrentUserService {
    ...
    public UUID userId() {
        SpotLinkPrincipal principal = principal();
        if (!users.existsByIdAndRegistrationStatus(principal.getUserId(), RegistrationStatus.ACTIVE)) {
            throw new BadCredentialsException("Authenticated user is no longer active.");
        }
        return principal.getUserId();
    }

    private SpotLinkPrincipal principal() {
