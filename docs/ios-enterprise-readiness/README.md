...
| Notifications | Delivery-ready foundation | Device-token lifecycle, backend APNs-ready provider boundary, server-side preference enforcement, metrics, and token redaction exist; Apple entitlements/credentials and physical-device delivery remain. |
| Security/privacy | Partially hardened | SMTP password reset delivery readiness, public auth/analytics rate limits, account-level auth lockout, first-party analytics privacy controls, token lifecycle, and privacy scaffolding exist; real SMTP credentials/sender-domain setup, provider/WAF abuse controls, owner-approved privacy policy/analytics consent copy, and production secrets are not complete. |
